...
import { h,memo,useCallback, useRef, useState, useMemo, useEffect,ComponentChild,VNode,RefObject,createRef } from './deps.ts';

import { Pane } from './Pane.tsx';
import { Resizer } from './Resizer.tsx';
import { ClientPosition, useDragState, DragState } from './util.ts';
...
function getNodeKey(node: ComponentChild, index: number): string {
	if (typeof node === 'object' && node && (node as any).key != null) {
		return 'key.' + (node as VNode).key;
	}
...
function useSplitPaneResize(options: SplitPaneResizeOptions): {
	...
	const paneRefs = useRef(new Map<string, RefObject<HTMLDivElement>>());

	const getMovedSizes = useCallback(
		(dragState: DragState<ResizeState> | null): number[] => {
			
			const collectedSizes = (children as ComponentChild[]).map(
				(node, index) =>
	...
	const childPanes = useMemo(() => {
		...
		return children.map((node, index) => {
			...
			const ref = prevPaneRefs.get(key) || createRef();
...
export const SplitPane = memo<SplitPaneProps>((props) => {
	...
	const splitStyleProps: h.JSX.CSSProperties =
		split === 'vertical'
			? {
					left: 0,
					right: 0,
	...
	const dragLayerStyle: h.JSX.CSSProperties = {
		position: 'absolute',
		top: 0,
		right: 0,
		bottom: 0,
